...
export class GameComponent implements OnInit, AfterViewInit {
	...
	private getLeaderboards() {
		this.leaderBoardService.allLeaderboards$.subscribe((leaderboards) => (this.leaderboards = leaderboards));
	}
	...
	private initSketch() {
		...
		this.sketch = (s) => {
			...
			s.draw = () => {
				...
				collectibles = collectibles.filter((o) => {
					...
					if (o.isCollided(player)) {
						...
						collectibleParticles.push(
							new CollectedParticle(s, pos, this.assetsManager.pointCollectedEffects)
						);
					}
				...
				if (s.frameCount % 245 === 0 && this.isStarted) {
					generateObstacles();
					generateCollectible();
				}
			};
			...
			let generateInitialObstacles = () => {
				...
				for (let i = 0; i < 2; i++) {
					...
					const obstacleObj = cloneDeep(this.assetsManager.obstacles[obstacleKey]);
					const pos = s.createVector(
						initialX,
						obstacleObj.dir === 'up' ? 10 : innerHeight - obstacleObj.originalSize[1] * this.scale
					);
			...
			let generateObstacles = () => {
				...
				const isCollided = obstacles.some((o) => o.isCollided(new Obstacle(s, pos, obstacleObj)));
				if (!isCollided) {
					obstacles.push(new Obstacle(s, pos, obstacleObj));
				}
			};
			...
			let initializeCollectibles = () => {
				...
				let initialSize = [ 148 * this.scale, 168.5 * this.scale ];
				let y;
				let iterations = 0;
				do {
				...
				collectibles.push(
					new Collectible(
						...
						this.assetsManager.collectibles[activeCollectibleIndex]
					)
				);
				activeCollectibleIndex = activeCollectibleIndex === 4 ? 0 : activeCollectibleIndex + 1;
			};
